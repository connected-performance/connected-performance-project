...
{    
    public function sale(Request $request)
    {
        ...
        try {
            ...
            if($webhook['event_body']["condition"]){ $condition=$webhook['event_body']["condition"]; }else{ $condition=null; }
            if($webhook['event_body']["authorization_code"]){ $authorization_code=$webhook['event_body']["authorization_code"]; }else{ $authorization_code=null; }
    ...
    public function updateCustomerNew($customer_vault_id, $customer_id)
    {
        ...
        if($customer->up_cus==0){
            ...
            if($response_g == 1){
                ...
                if($pay_tru==true){
                    $gw->doSaleCustomerVault($customer_vault_id, $plan_amount, $invoice->order_nmi);
                }
            }
