...
<div class="navbar-container d-flex content">
    ...
    <ul class="nav navbar-nav align-items-center ms-auto">
        ...
        @php
            ...
            $notifictions = [];
            if ($count > 0) {
                $notifictions = auth()->user()->user_notifications;
            }
        @endphp
        @if (auth()->user()->is_admin == true)
            <li class="nav-item dropdown dropdown-notification me-25">
                <a class="nav-link code-link" href="javascript:void(0);" data-bs-toggle="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-media dropdown-menu-end">
                    <li class="dropdown-menu-header">
                        <div class="dropdown-header d-flex">
                            ...
                            <div class="badge rounded-pill badge-light-success">{{ $count }} New</div>
                        </div>
                    </li>
                    <li class="scrollable-container media-list">
                        @foreach (@$notifictions as $value)
                            @if (@$value->status == '0')
                                <a class="d-flex" href="javascript:void(0)">
                                    <div class="list-item d-flex align-items-start">
                                        <div class="me-1">
                                            <div class="avatar">
                                                <img src="{{ @$value->notifications->user->avatar }}" alt="avatar"
                                                    width="32" height="32">
                                            </div>
                                        </div>
                                        <div class="list-item-body flex-grow-1">
                                            <p class="media-heading"><span
                                                    class="fw-bolder">{{ @$value->notifications->user->first_name }}</span>
                                            <p class="media-heading"><span
                                                    class="fw-bolder">{{ @$value->notifications->title }}</span>
                                            </p>
                                            <small
                                                class="notification-text">{{ @$value->notifications->description }}.</small>
                                        </div>
                                    </div>
                                </a>
                            @endif
                    ...
                    <li class="dropdown-menu-footer">
                        <a class="btn btn-success w-100" href="{{ route('notification.index') }}">Read all
                            notifications</a>
                    </li>
                </ul>
            </li>
        ...
        <li class="nav-item dropdown dropdown-user">
            ...
            <div class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdown-user">
                ...
                <a class="dropdown-item dropdown-item-nav"
                    href="{{ Route::has('user.profile') ? route('user.profile') : 'javascript:void(0)' }}">
                    <i class="me-50" data-feather="user"></i> Profile
                </a>
                {{-- @if (Auth::check() && Laravel\Jetstream\Jetstream::hasApiFeatures()) --}}
                {{-- <a class="dropdown-item-nav" href="{{ route('api-tokens.index') }}">
                ...
                {{-- <a class="dropdown-item-nav" href="{{ route('teams.create') }}">
                    <i class="me-50" data-feather="users"></i> Create New Team
                </a> --}}
                {{-- @endcan --}}
                <a class="dropdown-item dropdown-item-nav" href="{{ route('logout') }}">
